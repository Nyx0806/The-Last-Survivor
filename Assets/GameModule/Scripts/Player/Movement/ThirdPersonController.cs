    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void CameraRotation()
        {
            ...
            if (_input.look.sqrMagnitude >= _threshold && !LockCameraPosition)
            {
                ...
                float deltaTimeMultiplier = IsCurrentDeviceMouse ? 0.05f : Time.deltaTime;
        ...
        private void Move()
        {
            ...
            #if ENABLE_INPUT_SYSTEM
            // Ép code đọc trạng thái đè phím Shift trái (PC) hoặc cò trái (Gamepad)
            bool isSprinting = false;
            if (Keyboard.current != null && Keyboard.current.leftShiftKey.isPressed)
            {
                isSprinting = true;
            }
            else if (Gamepad.current != null && Gamepad.current.leftTrigger.isPressed)
            {
                isSprinting = true; // Hỗ trợ tay cầm (nếu bạn có dùng)
            }
            float targetSpeed = isSprinting ? SprintSpeed : MoveSpeed;
            #else
                    // Dành cho hệ thống Input cũ của Unity
